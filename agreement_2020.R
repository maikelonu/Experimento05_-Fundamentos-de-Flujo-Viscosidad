...
summary.agreement.Allowance <-
    function(object,dec,...){
        print <- cbind(round(object$Allowance$CCC,4),
                       round(object$Allowance$Precision,4),
                       round(object$Allowance$Accuracy,4),
                       round(object$Allowance$TDI,4),
                       round(object$Allowance$CP,4),
                       round(object$Allowance$RBS,4));
        colnames(print) <- c("CCC","Precision","Accuracy","TDI","CP","RBS");
        rownames(print) <- "";
        print(print,quote=FALSE, na.print=".",...);
    }

summary.agreement.Estimate <-
    function(object,dec,...){
        print <- cbind(round(object$Estimate$CCC,4),
                       round(object$Estimate$Precision,4),
                       round(object$Estimate$Accuracy,4),
                       round(object$Estimate$TDI,4),
                       round(object$Estimate$CP,4),
                       round(object$Estimate$RBS,4));
        colnames(print) <- c("CCC","Precision","Accuracy","TDI","CP","RBS");
        rownames(print) <- "";
        print(print,quote=FALSE, na.print=".",...);
    }

summary.agreement.Conf_Limit <-
    function(object,dec,...){
        print <- cbind(round(object$Conf_Limit$CCC,4),
                       round(object$Conf_Limit$Precision,4),
                       round(object$Conf_Limit$Accuracy,4),
                       round(object$Conf_Limit$TDI,4),
                       round(object$Conf_Limit$CP,4),
                       round(object$Conf_Limit$RBS,4));
        colnames(print) <- c("CCC","Precision","Accuracy","TDI","CP","RBS");
        rownames(print) <- "";
        print(print,quote=FALSE, na.print=".",...);
    }